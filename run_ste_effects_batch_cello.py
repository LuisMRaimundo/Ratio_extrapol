# -*- coding: utf-8 -*-
"""Batch STE Lab run: IOWA + ORCHIDEA for every cello effect we can ground in data."""
...
TREE = WORKSPACE / "CELLO"
OUT = WORKSPACE / "CELLO"
OPEN_C2 = 36
HARM_LO = 60  # C4 — first measured Orchidea chromatic harmonic
CELLO_CEILING = 84  # C6 — last arco Media note


def discover_research_files() -> list[dict]:
    specs = []
    warnings: list[str] = []
    warnings.extend(audit_iowa_zenodo_workbook(ARCO, media_sheet="Cello_Media"))
    trees = [
        (TREE / "Orchidea_cello" / "sul-ponticello", "ORCH", "sul ponticello"),
        (TREE / "Orchidea_cello" / "harmonics", "ORCH", "harmonics"),
        (TREE / "Orchidea_cello" / "con-sord", "ORCH", "con sordino"),
        (TREE / "Philharmonia_cello", "PHIL", None),
        (TREE / "McGill_cello", "MCGILL", None),
    ]
...
def main() -> None:
    print("Loading cello arco-normal Media…")
    ...
    for spec in specs:
        ...
        if spec["technique"] != "harmonics":
            mass = {m: v for m, v in mass.items() if m >= OPEN_C2}
    ...
    for effect in effects:
        print(f"\n=== {effect} ===")
        project = Project(
            title=f"Cello {effect} — STE Lab IOWA + ORCHIDEA",
            ...
            notes=(
                ...
                "Cello con sordino is measured at Orchidea pp, mf and ff."
            ),
        )
        ...
        for dyn in CORE_DYNS:
            iowa_arco_layer = build_layer(
                "cello", "IOWA", "ordinario", dyn, arco["IOWA"][dyn], "measured",
                "Cello_Media arco-normal", str(ARCO.name),
            )
            orch_arco_layer = build_layer(
                "cello", "ORCH", "ordinario", dyn, arco["ORCH"][dyn], "measured",
                "Cello_Media arco-normal", str(ARCO.name),
            )
            ...
            min_midi = HARM_LO if effect == "harmonics" else OPEN_C2
        ...
        for dyn, curve in phil_meas.items():
            ...
            project.add_layer(
                build_layer(
                    "cello", "PHIL", effect, dyn, curve, "measured",
        ...
        for dyn, curve in mcgill_meas.items():
            ...
            project.add_layer(
                build_layer(
                    "cello", "MCGILL", effect, dyn, curve, "measured",
        ...
        out = OUT / f"Cello_STE_{slug}_IOWA_ORCH.xlsx"
        ...
        zenodo = OUT / f"Cello_Zenodo_collections_{slug}.xlsx"
        export_zenodo_workbook(
            project,
            zenodo,
            technique=effect,
            arco=arco,
            ceiling_midi=CELLO_CEILING,
            notes=project.notes,
        )
